...
function getUnBlockedDoor(building,character)
			...
			if(sq) then
				...
				for j=0, Objs:size()-1 do
					...
					if(Object ~= nil) then
						local distance = getDistanceBetween(sq,character)
						if(instanceof(Object,"IsoDoor")) and (Object:isExteriorDoor(character)) and (distance < closestSoFar) then
							if(not Object:isLocked()) or (not Object:isBarricaded()) then
								closestSoFar = distance
								DoorOut = Object
							end
						end
...
function table.load( fileName )
	local fileTable = {}
	local readFile = getModFileReader("SubparSurvivors",getWorld():getWorld()..getFileSeparator()..fileName..".lua", true)
	if(readFile) then
